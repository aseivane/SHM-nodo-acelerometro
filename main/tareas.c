...
void leo_muestras(void *arg)
{
        ...
        while (1) {
                ...
                // microsToTimestamp(ttTime1, ticToctimestamp);
                // printf("TicTocTime %s (%lld) \n",ticToctimestamp,  ttTime1);
        ...
        while (1) {
                ...
                if( xSemaphore_guardatabla != NULL ) { //Chequea que el semáforo esté inicializado
                        ...
                        if( xSemaphoreTake( xSemaphore_guardatabla, 0 ) == pdTRUE ) //Si se guardó una tabla de datos se libera el semáforo
                        {
                                ...
                                nro_tabla++;  // Aumento contador de tablas guardadas
...
//fwrite(TABLA0, sizeof(uint8_t), sizeof(TABLA0), f_samples);
                                ...
                                if (f_samples == NULL) {
                                        ...
                                        if (nro_tabla > (TABLAS_POR_ARCHIVO-1)) { // Porque grabé en el archivo todas las tablas que tenia que guardar
                                                fclose(f_samples);
                                                nro_tabla = 0;  // Reinicio el contador de tablas en archivo
                                        }
...
void muestra_info(void *arg)
{
        while(1) {
                ...
                if (mensaje.mensaje_nuevo == true) {
                        mensaje.mensaje_nuevo=false;
